...
use super::trap_robber_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Trap Robber.nksf")
}

#[test]
fn test_trap_robber_parse_success() {
...
#[test]
fn test_trap_robber_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
