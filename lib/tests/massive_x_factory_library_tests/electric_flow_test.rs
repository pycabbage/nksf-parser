...
use super::electric_flow_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Electric Flow.nksf")
}

#[test]
fn test_electric_flow_parse_success() {
...
#[test]
fn test_electric_flow_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
