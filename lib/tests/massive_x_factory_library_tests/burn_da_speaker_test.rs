...
use super::burn_da_speaker_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Burn Da Speaker.nksf")
}

#[test]
fn test_burn_da_speaker_parse_success() {
...
#[test]
fn test_burn_da_speaker_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
