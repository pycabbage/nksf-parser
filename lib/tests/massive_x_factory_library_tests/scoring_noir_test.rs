...
use super::scoring_noir_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Scoring Noir.nksf")
}

#[test]
fn test_scoring_noir_parse_success() {
...
#[test]
fn test_scoring_noir_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
