...
use super::swoosh_swell_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Swoosh Swell.nksf")
}

#[test]
fn test_swoosh_swell_parse_success() {
...
#[test]
fn test_swoosh_swell_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
