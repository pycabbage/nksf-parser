...
use super::deep_fried_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Deep Fried.nksf")
}

#[test]
fn test_deep_fried_parse_success() {
...
#[test]
fn test_deep_fried_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
