...
use super::spring_coils_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Spring Coils.nksf")
}

#[test]
fn test_spring_coils_parse_success() {
...
#[test]
fn test_spring_coils_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
