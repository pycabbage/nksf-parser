...
use super::calm_rocket_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Calm Rocket.nksf")
}

#[test]
fn test_calm_rocket_parse_success() {
...
#[test]
fn test_calm_rocket_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
