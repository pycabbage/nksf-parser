...
use super::deep_growl_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Deep Growl.nksf")
}

#[test]
fn test_deep_growl_parse_success() {
...
#[test]
fn test_deep_growl_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
