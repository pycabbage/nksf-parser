...
use super::gamers_glitch_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Gamers Glitch.nksf")
}

#[test]
fn test_gamers_glitch_parse_success() {
...
#[test]
fn test_gamers_glitch_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
