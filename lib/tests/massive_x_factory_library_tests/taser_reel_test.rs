...
use super::taser_reel_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Taser Reel.nksf")
}

#[test]
fn test_taser_reel_parse_success() {
...
#[test]
fn test_taser_reel_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
