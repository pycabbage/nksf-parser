...
use super::ancient_strings_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ancient Strings.nksf")
}

#[test]
fn test_ancient_strings_parse_success() {
...
#[test]
fn test_ancient_strings_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
