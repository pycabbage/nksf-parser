...
use super::lazer_dust_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lazer Dust.nksf")
}

#[test]
fn test_lazer_dust_parse_success() {
...
#[test]
fn test_lazer_dust_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
