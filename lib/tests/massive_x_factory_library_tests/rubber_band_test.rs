...
use super::rubber_band_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Rubber Band.nksf")
}

#[test]
fn test_rubber_band_parse_success() {
...
#[test]
fn test_rubber_band_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
