...
use super::balearic_saw_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Balearic Saw.nksf")
}

#[test]
fn test_balearic_saw_parse_success() {
...
#[test]
fn test_balearic_saw_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
