...
use super::sagittarius_x_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sagittarius X.nksf")
}

#[test]
fn test_sagittarius_x_parse_success() {
...
#[test]
fn test_sagittarius_x_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
