...
use super::broken_arrow_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Broken Arrow.nksf")
}

#[test]
fn test_broken_arrow_parse_success() {
...
#[test]
fn test_broken_arrow_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
