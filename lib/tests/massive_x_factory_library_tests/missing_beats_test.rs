...
use super::missing_beats_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Missing Beats.nksf")
}

#[test]
fn test_missing_beats_parse_success() {
...
#[test]
fn test_missing_beats_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
