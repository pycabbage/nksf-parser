...
use super::alpine_digital_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Alpine Digital.nksf")
}

#[test]
fn test_alpine_digital_parse_success() {
...
#[test]
fn test_alpine_digital_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
