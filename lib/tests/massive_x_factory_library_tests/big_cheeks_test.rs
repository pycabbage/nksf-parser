...
use super::big_cheeks_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Big Cheeks.nksf")
}

#[test]
fn test_big_cheeks_parse_success() {
...
#[test]
fn test_big_cheeks_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
