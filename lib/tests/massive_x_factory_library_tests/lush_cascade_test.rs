...
use super::lush_cascade_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lush Cascade.nksf")
}

#[test]
fn test_lush_cascade_parse_success() {
...
#[test]
fn test_lush_cascade_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
