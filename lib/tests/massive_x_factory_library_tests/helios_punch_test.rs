...
use super::helios_punch_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Helios Punch.nksf")
}

#[test]
fn test_helios_punch_parse_success() {
...
#[test]
fn test_helios_punch_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
