...
use super::machine_kick_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Machine Kick.nksf")
}

#[test]
fn test_machine_kick_parse_success() {
...
#[test]
fn test_machine_kick_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
