...
use super::buried_giant_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Buried Giant.nksf")
}

#[test]
fn test_buried_giant_parse_success() {
...
#[test]
fn test_buried_giant_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
