...
use super::retro_rise_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Retro Rise.nksf")
}

#[test]
fn test_retro_rise_parse_success() {
...
#[test]
fn test_retro_rise_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
