...
use super::texture_patch_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Texture Patch.nksf")
}

#[test]
fn test_texture_patch_parse_success() {
...
#[test]
fn test_texture_patch_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
