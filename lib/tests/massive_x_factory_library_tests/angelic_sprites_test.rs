...
use super::angelic_sprites_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Angelic Sprites.nksf")
}

#[test]
fn test_angelic_sprites_parse_success() {
...
#[test]
fn test_angelic_sprites_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
