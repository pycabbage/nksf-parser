...
use super::crispy_lamb_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Crispy Lamb.nksf")
}

#[test]
fn test_crispy_lamb_parse_success() {
...
#[test]
fn test_crispy_lamb_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
