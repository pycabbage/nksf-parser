...
use super::essex_shuffla_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Essex Shuffla.nksf")
}

#[test]
fn test_essex_shuffla_parse_success() {
...
#[test]
fn test_essex_shuffla_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
