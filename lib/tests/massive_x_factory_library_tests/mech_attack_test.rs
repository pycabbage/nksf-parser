...
use super::mech_attack_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mech Attack.nksf")
}

#[test]
fn test_mech_attack_parse_success() {
...
#[test]
fn test_mech_attack_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
