...
use super::noise_pulse_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Noise Pulse.nksf")
}

#[test]
fn test_noise_pulse_parse_success() {
...
#[test]
fn test_noise_pulse_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
