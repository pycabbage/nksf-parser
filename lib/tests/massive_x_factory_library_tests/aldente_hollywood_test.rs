...
use super::aldente_hollywood_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Aldente Hollywood.nksf")
}

#[test]
fn test_aldente_hollywood_parse_success() {
...
#[test]
fn test_aldente_hollywood_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_aldente_hollywood_nisi() {
    let path = get_fixture_path();
