...
use super::coldest_champion_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Coldest Champion.nksf")
}

#[test]
fn test_coldest_champion_parse_success() {
...
#[test]
fn test_coldest_champion_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
