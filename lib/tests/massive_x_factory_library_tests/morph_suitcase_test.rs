...
use super::morph_suitcase_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Morph Suitcase.nksf")
}

#[test]
fn test_morph_suitcase_parse_success() {
...
#[test]
fn test_morph_suitcase_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
