...
use super::now_dystopia_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Now Dystopia.nksf")
}

#[test]
fn test_now_dystopia_parse_success() {
...
#[test]
fn test_now_dystopia_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
