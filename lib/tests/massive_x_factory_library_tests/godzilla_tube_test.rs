...
use super::godzilla_tube_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Godzilla Tube.nksf")
}

#[test]
fn test_godzilla_tube_parse_success() {
...
#[test]
fn test_godzilla_tube_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
