...
use super::clean_strings_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Clean Strings.nksf")
}

#[test]
fn test_clean_strings_parse_success() {
...
#[test]
fn test_clean_strings_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
