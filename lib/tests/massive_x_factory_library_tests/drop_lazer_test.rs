...
use super::drop_lazer_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Drop Lazer.nksf")
}

#[test]
fn test_drop_lazer_parse_success() {
...
#[test]
fn test_drop_lazer_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
