...
use super::kingson_brown_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Kingson Brown.nksf")
}

#[test]
fn test_kingson_brown_parse_success() {
...
#[test]
fn test_kingson_brown_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
