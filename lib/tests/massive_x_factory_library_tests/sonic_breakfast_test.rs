...
use super::sonic_breakfast_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sonic Breakfast.nksf")
}

#[test]
fn test_sonic_breakfast_parse_success() {
...
#[test]
fn test_sonic_breakfast_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
