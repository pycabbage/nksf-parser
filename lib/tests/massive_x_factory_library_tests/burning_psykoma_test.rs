...
use super::burning_psykoma_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Burning Psykoma.nksf")
}

#[test]
fn test_burning_psykoma_parse_success() {
...
#[test]
fn test_burning_psykoma_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
