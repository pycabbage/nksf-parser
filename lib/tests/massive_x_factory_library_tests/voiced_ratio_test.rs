...
use super::voiced_ratio_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Voiced Ratio.nksf")
}

#[test]
fn test_voiced_ratio_parse_success() {
...
#[test]
fn test_voiced_ratio_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
