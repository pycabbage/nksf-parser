...
use super::splitting_atoms_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Splitting Atoms.nksf")
}

#[test]
fn test_splitting_atoms_parse_success() {
...
#[test]
fn test_splitting_atoms_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
