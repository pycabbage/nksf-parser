...
use super::retaliation_is_a_must_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Retaliation Is A Must.nksf")
}

#[test]
fn test_retaliation_is_a_must_parse_success() {
...
#[test]
fn test_retaliation_is_a_must_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_retaliation_is_a_must_nisi() {
    let path = get_fixture_path();
