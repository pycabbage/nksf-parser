...
use super::pastoral_landscape_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Pastoral Landscape.nksf")
}

#[test]
fn test_pastoral_landscape_parse_success() {
...
#[test]
fn test_pastoral_landscape_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_pastoral_landscape_nisi() {
    let path = get_fixture_path();
