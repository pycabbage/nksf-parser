...
use super::insane_membrane_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Insane Membrane.nksf")
}

#[test]
fn test_insane_membrane_parse_success() {
...
#[test]
fn test_insane_membrane_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
