...
use super::eighties_essential_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Eighties Essential.nksf")
}

#[test]
fn test_eighties_essential_parse_success() {
...
#[test]
fn test_eighties_essential_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_eighties_essential_nisi() {
    let path = get_fixture_path();
