...
use super::fruit_walk_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Fruit Walk.nksf")
}

#[test]
fn test_fruit_walk_parse_success() {
...
#[test]
fn test_fruit_walk_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
