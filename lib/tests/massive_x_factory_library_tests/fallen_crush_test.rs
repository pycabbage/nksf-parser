...
use super::fallen_crush_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Fallen Crush.nksf")
}

#[test]
fn test_fallen_crush_parse_success() {
...
#[test]
fn test_fallen_crush_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
