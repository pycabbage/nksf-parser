...
use super::riser_collection_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Riser Collection.nksf")
}

#[test]
fn test_riser_collection_parse_success() {
...
#[test]
fn test_riser_collection_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
