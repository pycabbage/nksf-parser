...
use super::tuby_tuesday_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Tuby Tuesday.nksf")
}

#[test]
fn test_tuby_tuesday_parse_success() {
...
#[test]
fn test_tuby_tuesday_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
