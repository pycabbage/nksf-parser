...
use super::organ_gator_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Organ Gator.nksf")
}

#[test]
fn test_organ_gator_parse_success() {
...
#[test]
fn test_organ_gator_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
