...
use super::funk_glider_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Funk Glider.nksf")
}

#[test]
fn test_funk_glider_parse_success() {
...
#[test]
fn test_funk_glider_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
