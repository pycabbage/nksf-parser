...
use super::junox_poly_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Junox Poly.nksf")
}

#[test]
fn test_junox_poly_parse_success() {
...
#[test]
fn test_junox_poly_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
