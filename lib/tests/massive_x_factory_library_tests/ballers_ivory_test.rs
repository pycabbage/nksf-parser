...
use super::ballers_ivory_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ballers Ivory.nksf")
}

#[test]
fn test_ballers_ivory_parse_success() {
...
#[test]
fn test_ballers_ivory_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
