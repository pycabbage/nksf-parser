...
use super::vivid_voyager_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Vivid Voyager.nksf")
}

#[test]
fn test_vivid_voyager_parse_success() {
...
#[test]
fn test_vivid_voyager_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
