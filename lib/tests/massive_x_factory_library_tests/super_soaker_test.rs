...
use super::super_soaker_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Super Soaker.nksf")
}

#[test]
fn test_super_soaker_parse_success() {
...
#[test]
fn test_super_soaker_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
