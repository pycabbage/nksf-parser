...
use super::green_pastures_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Green Pastures.nksf")
}

#[test]
fn test_green_pastures_parse_success() {
...
#[test]
fn test_green_pastures_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
