...
use super::stellar_drift_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Stellar Drift.nksf")
}

#[test]
fn test_stellar_drift_parse_success() {
...
#[test]
fn test_stellar_drift_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
