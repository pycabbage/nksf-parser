...
use super::primal_leader_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Primal Leader.nksf")
}

#[test]
fn test_primal_leader_parse_success() {
...
#[test]
fn test_primal_leader_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
