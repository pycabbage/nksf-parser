...
use super::mystic_visions_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mystic Visions.nksf")
}

#[test]
fn test_mystic_visions_parse_success() {
...
#[test]
fn test_mystic_visions_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
