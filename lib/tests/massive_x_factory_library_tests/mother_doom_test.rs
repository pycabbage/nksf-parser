...
use super::mother_doom_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mother Doom.nksf")
}

#[test]
fn test_mother_doom_parse_success() {
...
#[test]
fn test_mother_doom_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
