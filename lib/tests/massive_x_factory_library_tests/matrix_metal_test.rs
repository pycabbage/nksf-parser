...
use super::matrix_metal_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Matrix Metal.nksf")
}

#[test]
fn test_matrix_metal_parse_success() {
...
#[test]
fn test_matrix_metal_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
