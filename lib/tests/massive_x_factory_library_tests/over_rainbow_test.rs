...
use super::over_rainbow_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Over Rainbow.nksf")
}

#[test]
fn test_over_rainbow_parse_success() {
...
#[test]
fn test_over_rainbow_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
