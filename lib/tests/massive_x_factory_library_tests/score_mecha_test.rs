...
use super::score_mecha_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Score Mecha.nksf")
}

#[test]
fn test_score_mecha_parse_success() {
...
#[test]
fn test_score_mecha_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
