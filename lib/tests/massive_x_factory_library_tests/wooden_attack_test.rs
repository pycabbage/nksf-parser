...
use super::wooden_attack_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wooden Attack.nksf")
}

#[test]
fn test_wooden_attack_parse_success() {
...
#[test]
fn test_wooden_attack_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
