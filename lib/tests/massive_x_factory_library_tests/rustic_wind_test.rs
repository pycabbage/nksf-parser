...
use super::rustic_wind_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Rustic Wind.nksf")
}

#[test]
fn test_rustic_wind_parse_success() {
...
#[test]
fn test_rustic_wind_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
