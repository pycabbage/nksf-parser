...
use super::lets_dance_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lets Dance.nksf")
}

#[test]
fn test_lets_dance_parse_success() {
...
#[test]
fn test_lets_dance_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
