...
use super::sentient_mainframe_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sentient Mainframe.nksf")
}

#[test]
fn test_sentient_mainframe_parse_success() {
...
#[test]
fn test_sentient_mainframe_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_sentient_mainframe_nisi() {
    let path = get_fixture_path();
