...
use super::xberg_wunder_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Xberg Wunder.nksf")
}

#[test]
fn test_xberg_wunder_parse_success() {
...
#[test]
fn test_xberg_wunder_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
