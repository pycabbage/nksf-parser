...
use super::turbo_boost_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Turbo Boost.nksf")
}

#[test]
fn test_turbo_boost_parse_success() {
...
#[test]
fn test_turbo_boost_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
