...
use super::organic_closer_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Organic Closer.nksf")
}

#[test]
fn test_organic_closer_parse_success() {
...
#[test]
fn test_organic_closer_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
