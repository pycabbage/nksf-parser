...
use super::maximus_brute_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Maximus Brute.nksf")
}

#[test]
fn test_maximus_brute_parse_success() {
...
#[test]
fn test_maximus_brute_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
