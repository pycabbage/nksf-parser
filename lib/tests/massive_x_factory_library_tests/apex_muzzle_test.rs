...
use super::apex_muzzle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Apex Muzzle.nksf")
}

#[test]
fn test_apex_muzzle_parse_success() {
...
#[test]
fn test_apex_muzzle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
