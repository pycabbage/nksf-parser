...
use super::sprinkle_pad_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sprinkle Pad.nksf")
}

#[test]
fn test_sprinkle_pad_parse_success() {
...
#[test]
fn test_sprinkle_pad_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
