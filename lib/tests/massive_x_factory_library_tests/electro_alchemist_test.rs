...
use super::electro_alchemist_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Electro Alchemist.nksf")
}

#[test]
fn test_electro_alchemist_parse_success() {
...
#[test]
fn test_electro_alchemist_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_electro_alchemist_nisi() {
    let path = get_fixture_path();
