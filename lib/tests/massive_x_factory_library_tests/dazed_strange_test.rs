...
use super::dazed_strange_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dazed Strange.nksf")
}

#[test]
fn test_dazed_strange_parse_success() {
...
#[test]
fn test_dazed_strange_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
