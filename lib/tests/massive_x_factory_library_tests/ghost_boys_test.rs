...
use super::ghost_boys_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ghost Boys.nksf")
}

#[test]
fn test_ghost_boys_parse_success() {
...
#[test]
fn test_ghost_boys_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
