...
use super::wisdom_oracle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wisdom Oracle.nksf")
}

#[test]
fn test_wisdom_oracle_parse_success() {
...
#[test]
fn test_wisdom_oracle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
