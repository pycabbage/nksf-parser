...
use super::minor_rise_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Minor Rise.nksf")
}

#[test]
fn test_minor_rise_parse_success() {
...
#[test]
fn test_minor_rise_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
