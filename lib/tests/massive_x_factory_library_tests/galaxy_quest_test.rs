...
use super::galaxy_quest_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Galaxy Quest.nksf")
}

#[test]
fn test_galaxy_quest_parse_success() {
...
#[test]
fn test_galaxy_quest_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
