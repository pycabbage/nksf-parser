...
use super::quantum_foam_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Quantum Foam.nksf")
}

#[test]
fn test_quantum_foam_parse_success() {
...
#[test]
fn test_quantum_foam_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
