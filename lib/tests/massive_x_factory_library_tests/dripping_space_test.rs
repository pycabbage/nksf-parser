...
use super::dripping_space_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dripping Space.nksf")
}

#[test]
fn test_dripping_space_parse_success() {
...
#[test]
fn test_dripping_space_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
