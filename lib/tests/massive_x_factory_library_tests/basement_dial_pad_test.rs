...
use super::basement_dial_pad_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Basement Dial Pad.nksf")
}

#[test]
fn test_basement_dial_pad_parse_success() {
...
#[test]
fn test_basement_dial_pad_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_basement_dial_pad_nisi() {
    let path = get_fixture_path();
