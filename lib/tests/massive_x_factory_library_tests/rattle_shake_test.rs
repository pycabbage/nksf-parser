...
use super::rattle_shake_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Rattle Shake.nksf")
}

#[test]
fn test_rattle_shake_parse_success() {
...
#[test]
fn test_rattle_shake_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
