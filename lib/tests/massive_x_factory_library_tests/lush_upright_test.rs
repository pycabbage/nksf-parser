...
use super::lush_upright_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lush Upright.nksf")
}

#[test]
fn test_lush_upright_parse_success() {
...
#[test]
fn test_lush_upright_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
