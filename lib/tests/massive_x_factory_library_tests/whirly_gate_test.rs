...
use super::whirly_gate_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Whirly Gate.nksf")
}

#[test]
fn test_whirly_gate_parse_success() {
...
#[test]
fn test_whirly_gate_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
