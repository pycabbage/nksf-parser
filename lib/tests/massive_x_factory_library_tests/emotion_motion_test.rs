...
use super::emotion_motion_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Emotion Motion.nksf")
}

#[test]
fn test_emotion_motion_parse_success() {
...
#[test]
fn test_emotion_motion_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
