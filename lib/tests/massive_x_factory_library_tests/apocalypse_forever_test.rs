...
use super::apocalypse_forever_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Apocalypse Forever.nksf")
}

#[test]
fn test_apocalypse_forever_parse_success() {
...
#[test]
fn test_apocalypse_forever_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_apocalypse_forever_nisi() {
    let path = get_fixture_path();
