...
use super::inner_nature_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Inner Nature.nksf")
}

#[test]
fn test_inner_nature_parse_success() {
...
#[test]
fn test_inner_nature_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
