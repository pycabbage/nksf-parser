...
use super::one_patch_to_rule_them_all_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("One Patch to Rule Them All.nksf")
}

#[test]
fn test_one_patch_to_rule_them_all_parse_success() {
...
#[test]
fn test_one_patch_to_rule_them_all_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_one_patch_to_rule_them_all_nisi() {
    let path = get_fixture_path();
