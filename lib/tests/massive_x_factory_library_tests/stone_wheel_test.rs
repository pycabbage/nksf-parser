...
use super::stone_wheel_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Stone Wheel.nksf")
}

#[test]
fn test_stone_wheel_parse_success() {
...
#[test]
fn test_stone_wheel_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
