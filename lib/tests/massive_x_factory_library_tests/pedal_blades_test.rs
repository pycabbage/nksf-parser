...
use super::pedal_blades_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Pedal Blades.nksf")
}

#[test]
fn test_pedal_blades_parse_success() {
...
#[test]
fn test_pedal_blades_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
