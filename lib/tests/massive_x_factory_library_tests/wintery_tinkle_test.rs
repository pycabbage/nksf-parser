...
use super::wintery_tinkle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wintery Tinkle.nksf")
}

#[test]
fn test_wintery_tinkle_parse_success() {
...
#[test]
fn test_wintery_tinkle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
