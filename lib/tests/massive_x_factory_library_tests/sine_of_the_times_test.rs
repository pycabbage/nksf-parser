...
use super::sine_of_the_times_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sine Of The Times.nksf")
}

#[test]
fn test_sine_of_the_times_parse_success() {
...
#[test]
fn test_sine_of_the_times_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_sine_of_the_times_nisi() {
    let path = get_fixture_path();
