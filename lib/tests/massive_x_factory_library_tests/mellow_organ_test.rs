...
use super::mellow_organ_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mellow Organ.nksf")
}

#[test]
fn test_mellow_organ_parse_success() {
...
#[test]
fn test_mellow_organ_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
