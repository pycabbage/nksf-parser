...
use super::sunny_island_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sunny Island.nksf")
}

#[test]
fn test_sunny_island_parse_success() {
...
#[test]
fn test_sunny_island_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
