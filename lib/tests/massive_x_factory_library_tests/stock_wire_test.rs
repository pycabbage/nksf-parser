...
use super::stock_wire_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Stock Wire.nksf")
}

#[test]
fn test_stock_wire_parse_success() {
...
#[test]
fn test_stock_wire_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
