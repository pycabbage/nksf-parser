...
use super::spiral_lead_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Spiral Lead.nksf")
}

#[test]
fn test_spiral_lead_parse_success() {
...
#[test]
fn test_spiral_lead_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
