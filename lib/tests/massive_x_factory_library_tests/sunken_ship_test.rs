...
use super::sunken_ship_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sunken Ship.nksf")
}

#[test]
fn test_sunken_ship_parse_success() {
...
#[test]
fn test_sunken_ship_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
