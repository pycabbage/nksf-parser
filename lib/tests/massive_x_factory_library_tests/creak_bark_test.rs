...
use super::creak_bark_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Creak Bark.nksf")
}

#[test]
fn test_creak_bark_parse_success() {
...
#[test]
fn test_creak_bark_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
