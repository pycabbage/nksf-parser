...
use super::sub_breese_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sub Breese.nksf")
}

#[test]
fn test_sub_breese_parse_success() {
...
#[test]
fn test_sub_breese_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
