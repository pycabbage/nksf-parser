...
use super::alkaline_state_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Alkaline State.nksf")
}

#[test]
fn test_alkaline_state_parse_success() {
...
#[test]
fn test_alkaline_state_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
