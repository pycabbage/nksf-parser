...
use super::string_bundle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("String Bundle.nksf")
}

#[test]
fn test_string_bundle_parse_success() {
...
#[test]
fn test_string_bundle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
