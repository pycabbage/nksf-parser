...
use super::uncle_john_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Uncle John.nksf")
}

#[test]
fn test_uncle_john_parse_success() {
...
#[test]
fn test_uncle_john_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
