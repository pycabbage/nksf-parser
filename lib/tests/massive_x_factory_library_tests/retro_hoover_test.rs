...
use super::retro_hoover_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Retro Hoover.nksf")
}

#[test]
fn test_retro_hoover_parse_success() {
...
#[test]
fn test_retro_hoover_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
