...
use super::acid_lazers_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Acid Lazers.nksf")
}

#[test]
fn test_acid_lazers_parse_success() {
...
#[test]
fn test_acid_lazers_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
