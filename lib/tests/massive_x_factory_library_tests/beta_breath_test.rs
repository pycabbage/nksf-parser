...
use super::beta_breath_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Beta Breath.nksf")
}

#[test]
fn test_beta_breath_parse_success() {
...
#[test]
fn test_beta_breath_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
