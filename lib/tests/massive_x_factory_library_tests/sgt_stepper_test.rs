...
use super::sgt_stepper_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sgt Stepper.nksf")
}

#[test]
fn test_sgt_stepper_parse_success() {
...
#[test]
fn test_sgt_stepper_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
