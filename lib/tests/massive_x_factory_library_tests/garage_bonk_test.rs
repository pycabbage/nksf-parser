...
use super::garage_bonk_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Garage Bonk.nksf")
}

#[test]
fn test_garage_bonk_parse_success() {
...
#[test]
fn test_garage_bonk_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
