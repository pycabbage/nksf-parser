...
use super::massive_impact_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Massive Impact.nksf")
}

#[test]
fn test_massive_impact_parse_success() {
...
#[test]
fn test_massive_impact_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
