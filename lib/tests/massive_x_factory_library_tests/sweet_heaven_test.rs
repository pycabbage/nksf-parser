...
use super::sweet_heaven_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sweet Heaven.nksf")
}

#[test]
fn test_sweet_heaven_parse_success() {
...
#[test]
fn test_sweet_heaven_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
