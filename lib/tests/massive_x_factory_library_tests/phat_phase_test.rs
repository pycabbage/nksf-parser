...
use super::phat_phase_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Phat Phase.nksf")
}

#[test]
fn test_phat_phase_parse_success() {
...
#[test]
fn test_phat_phase_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
