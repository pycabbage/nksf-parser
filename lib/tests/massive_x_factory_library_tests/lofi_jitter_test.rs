...
use super::lofi_jitter_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lofi Jitter.nksf")
}

#[test]
fn test_lofi_jitter_parse_success() {
...
#[test]
fn test_lofi_jitter_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
