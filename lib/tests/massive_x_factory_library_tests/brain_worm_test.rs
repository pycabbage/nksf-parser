...
use super::brain_worm_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Brain Worm.nksf")
}

#[test]
fn test_brain_worm_parse_success() {
...
#[test]
fn test_brain_worm_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
