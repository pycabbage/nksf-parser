...
use super::foundation_focus_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Foundation Focus.nksf")
}

#[test]
fn test_foundation_focus_parse_success() {
...
#[test]
fn test_foundation_focus_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
