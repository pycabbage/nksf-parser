...
use super::hunting_game_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Hunting Game.nksf")
}

#[test]
fn test_hunting_game_parse_success() {
...
#[test]
fn test_hunting_game_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
