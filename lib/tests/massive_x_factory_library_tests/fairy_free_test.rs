...
use super::fairy_free_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Fairy Free.nksf")
}

#[test]
fn test_fairy_free_parse_success() {
...
#[test]
fn test_fairy_free_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
