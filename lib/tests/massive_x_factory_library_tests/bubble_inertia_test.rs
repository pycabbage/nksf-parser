...
use super::bubble_inertia_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Bubble Inertia.nksf")
}

#[test]
fn test_bubble_inertia_parse_success() {
...
#[test]
fn test_bubble_inertia_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
