...
use super::little_beeper_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Little Beeper.nksf")
}

#[test]
fn test_little_beeper_parse_success() {
...
#[test]
fn test_little_beeper_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
