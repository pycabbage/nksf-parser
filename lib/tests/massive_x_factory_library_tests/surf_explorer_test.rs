...
use super::surf_explorer_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Surf Explorer.nksf")
}

#[test]
fn test_surf_explorer_parse_success() {
...
#[test]
fn test_surf_explorer_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
