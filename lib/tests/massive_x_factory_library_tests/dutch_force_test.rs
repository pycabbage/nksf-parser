...
use super::dutch_force_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dutch Force.nksf")
}

#[test]
fn test_dutch_force_parse_success() {
...
#[test]
fn test_dutch_force_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
