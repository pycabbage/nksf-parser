...
use super::easy_pulse_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Easy Pulse.nksf")
}

#[test]
fn test_easy_pulse_parse_success() {
...
#[test]
fn test_easy_pulse_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
