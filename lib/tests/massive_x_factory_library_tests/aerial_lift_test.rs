...
use super::aerial_lift_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Aerial Lift.nksf")
}

#[test]
fn test_aerial_lift_parse_success() {
...
#[test]
fn test_aerial_lift_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
