...
use super::super_corrupt_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Super Corrupt.nksf")
}

#[test]
fn test_super_corrupt_parse_success() {
...
#[test]
fn test_super_corrupt_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
