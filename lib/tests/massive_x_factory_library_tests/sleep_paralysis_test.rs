...
use super::sleep_paralysis_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sleep Paralysis.nksf")
}

#[test]
fn test_sleep_paralysis_parse_success() {
...
#[test]
fn test_sleep_paralysis_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
