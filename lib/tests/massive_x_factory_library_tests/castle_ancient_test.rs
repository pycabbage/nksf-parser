...
use super::castle_ancient_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Castle Ancient.nksf")
}

#[test]
fn test_castle_ancient_parse_success() {
...
#[test]
fn test_castle_ancient_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
