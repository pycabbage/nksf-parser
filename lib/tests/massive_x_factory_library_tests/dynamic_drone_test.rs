...
use super::dynamic_drone_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dynamic Drone.nksf")
}

#[test]
fn test_dynamic_drone_parse_success() {
...
#[test]
fn test_dynamic_drone_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
