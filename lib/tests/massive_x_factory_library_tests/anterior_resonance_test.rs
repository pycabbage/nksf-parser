...
use super::anterior_resonance_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Anterior Resonance.nksf")
}

#[test]
fn test_anterior_resonance_parse_success() {
...
#[test]
fn test_anterior_resonance_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_anterior_resonance_nisi() {
    let path = get_fixture_path();
