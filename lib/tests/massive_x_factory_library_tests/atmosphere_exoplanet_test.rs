// Atmosphere Exoplanet
// 自動生成されたテストファイル

use nksf_parser::{ParseError, parse_nksf};
use std::path::PathBuf;

use super::atmosphere_exoplanet_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Atmosphere Exoplanet.nksf")
}

#[test]
fn test_atmosphere_exoplanet_parse_success() {
...
#[test]
fn test_atmosphere_exoplanet_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_atmosphere_exoplanet_nisi() {
    let path = get_fixture_path();
