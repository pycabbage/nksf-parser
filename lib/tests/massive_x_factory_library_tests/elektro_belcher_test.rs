...
use super::elektro_belcher_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Elektro Belcher.nksf")
}

#[test]
fn test_elektro_belcher_parse_success() {
...
#[test]
fn test_elektro_belcher_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
