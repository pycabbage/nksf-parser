...
use super::scream_snare_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Scream Snare.nksf")
}

#[test]
fn test_scream_snare_parse_success() {
...
#[test]
fn test_scream_snare_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
