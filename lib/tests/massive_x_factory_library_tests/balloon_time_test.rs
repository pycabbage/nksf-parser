...
use super::balloon_time_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Balloon Time.nksf")
}

#[test]
fn test_balloon_time_parse_success() {
...
#[test]
fn test_balloon_time_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
