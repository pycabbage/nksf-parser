...
use super::cowboy_sync_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Cowboy Sync.nksf")
}

#[test]
fn test_cowboy_sync_parse_success() {
...
#[test]
fn test_cowboy_sync_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
