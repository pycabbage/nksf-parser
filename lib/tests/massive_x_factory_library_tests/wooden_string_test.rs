...
use super::wooden_string_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wooden String.nksf")
}

#[test]
fn test_wooden_string_parse_success() {
...
#[test]
fn test_wooden_string_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
