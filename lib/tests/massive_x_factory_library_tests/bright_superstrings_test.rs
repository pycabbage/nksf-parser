...
use super::bright_superstrings_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Bright Superstrings.nksf")
}

#[test]
fn test_bright_superstrings_parse_success() {
...
#[test]
fn test_bright_superstrings_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_bright_superstrings_nisi() {
    let path = get_fixture_path();
