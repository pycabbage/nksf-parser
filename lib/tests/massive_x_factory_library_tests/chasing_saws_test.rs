...
use super::chasing_saws_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Chasing Saws.nksf")
}

#[test]
fn test_chasing_saws_parse_success() {
...
#[test]
fn test_chasing_saws_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
