...
use super::robot_wars_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Robot Wars.nksf")
}

#[test]
fn test_robot_wars_parse_success() {
...
#[test]
fn test_robot_wars_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
