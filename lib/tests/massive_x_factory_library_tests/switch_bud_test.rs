...
use super::switch_bud_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Switch Bud.nksf")
}

#[test]
fn test_switch_bud_parse_success() {
...
#[test]
fn test_switch_bud_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
