...
use super::grime_knife_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Grime Knife.nksf")
}

#[test]
fn test_grime_knife_parse_success() {
...
#[test]
fn test_grime_knife_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
