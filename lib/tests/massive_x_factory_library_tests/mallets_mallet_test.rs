...
use super::mallets_mallet_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mallets Mallet.nksf")
}

#[test]
fn test_mallets_mallet_parse_success() {
...
#[test]
fn test_mallets_mallet_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
