...
use super::analog_tapestry_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Analog Tapestry.nksf")
}

#[test]
fn test_analog_tapestry_parse_success() {
...
#[test]
fn test_analog_tapestry_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
