...
use super::transient_steel_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Transient Steel.nksf")
}

#[test]
fn test_transient_steel_parse_success() {
...
#[test]
fn test_transient_steel_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
