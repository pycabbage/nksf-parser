...
use super::dunkin_bass_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dunkin Bass.nksf")
}

#[test]
fn test_dunkin_bass_parse_success() {
...
#[test]
fn test_dunkin_bass_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
