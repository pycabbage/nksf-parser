...
use super::fuzz_action_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Fuzz Action.nksf")
}

#[test]
fn test_fuzz_action_parse_success() {
...
#[test]
fn test_fuzz_action_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
