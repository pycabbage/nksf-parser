...
use super::wob_repeat_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wob Repeat.nksf")
}

#[test]
fn test_wob_repeat_parse_success() {
...
#[test]
fn test_wob_repeat_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
