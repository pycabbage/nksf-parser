...
use super::cloud_breaker_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Cloud Breaker.nksf")
}

#[test]
fn test_cloud_breaker_parse_success() {
...
#[test]
fn test_cloud_breaker_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
