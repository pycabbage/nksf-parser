...
use super::arena_hands_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Arena Hands.nksf")
}

#[test]
fn test_arena_hands_parse_success() {
...
#[test]
fn test_arena_hands_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
