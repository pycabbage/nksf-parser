...
use super::sand_steps_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sand Steps.nksf")
}

#[test]
fn test_sand_steps_parse_success() {
...
#[test]
fn test_sand_steps_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
