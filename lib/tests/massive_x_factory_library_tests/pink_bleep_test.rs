...
use super::pink_bleep_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Pink Bleep.nksf")
}

#[test]
fn test_pink_bleep_parse_success() {
...
#[test]
fn test_pink_bleep_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
