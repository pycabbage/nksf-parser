...
use super::racer_raver_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Racer Raver.nksf")
}

#[test]
fn test_racer_raver_parse_success() {
...
#[test]
fn test_racer_raver_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
