...
use super::anchor_minor_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Anchor Minor.nksf")
}

#[test]
fn test_anchor_minor_parse_success() {
...
#[test]
fn test_anchor_minor_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
