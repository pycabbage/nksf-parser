...
use super::alternate_tone_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Alternate Tone.nksf")
}

#[test]
fn test_alternate_tone_parse_success() {
...
#[test]
fn test_alternate_tone_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
