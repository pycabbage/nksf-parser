...
use super::wild_pitch_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wild Pitch.nksf")
}

#[test]
fn test_wild_pitch_parse_success() {
...
#[test]
fn test_wild_pitch_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
