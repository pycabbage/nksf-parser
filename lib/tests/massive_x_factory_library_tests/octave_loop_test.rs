...
use super::octave_loop_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Octave Loop.nksf")
}

#[test]
fn test_octave_loop_parse_success() {
...
#[test]
fn test_octave_loop_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
