...
use super::apricot_pop_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Apricot Pop.nksf")
}

#[test]
fn test_apricot_pop_parse_success() {
...
#[test]
fn test_apricot_pop_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
