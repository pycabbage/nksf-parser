...
use super::optical_tension_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Optical Tension.nksf")
}

#[test]
fn test_optical_tension_parse_success() {
...
#[test]
fn test_optical_tension_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
