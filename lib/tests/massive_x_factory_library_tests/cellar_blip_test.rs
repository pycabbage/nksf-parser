...
use super::cellar_blip_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Cellar Blip.nksf")
}

#[test]
fn test_cellar_blip_parse_success() {
...
#[test]
fn test_cellar_blip_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
