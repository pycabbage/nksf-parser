...
use super::rising_sun_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Rising Sun.nksf")
}

#[test]
fn test_rising_sun_parse_success() {
...
#[test]
fn test_rising_sun_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
