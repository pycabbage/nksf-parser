...
use super::comb_experiment_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Comb Experiment.nksf")
}

#[test]
fn test_comb_experiment_parse_success() {
...
#[test]
fn test_comb_experiment_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
