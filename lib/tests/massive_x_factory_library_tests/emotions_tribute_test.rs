...
use super::emotions_tribute_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Emotions Tribute.nksf")
}

#[test]
fn test_emotions_tribute_parse_success() {
...
#[test]
fn test_emotions_tribute_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
