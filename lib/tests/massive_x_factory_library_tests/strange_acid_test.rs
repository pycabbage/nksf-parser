...
use super::strange_acid_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Strange Acid.nksf")
}

#[test]
fn test_strange_acid_parse_success() {
...
#[test]
fn test_strange_acid_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
