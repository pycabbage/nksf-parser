...
use super::hollow_talk_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Hollow Talk.nksf")
}

#[test]
fn test_hollow_talk_parse_success() {
...
#[test]
fn test_hollow_talk_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
