...
use super::mild_sauce_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Mild Sauce.nksf")
}

#[test]
fn test_mild_sauce_parse_success() {
...
#[test]
fn test_mild_sauce_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
