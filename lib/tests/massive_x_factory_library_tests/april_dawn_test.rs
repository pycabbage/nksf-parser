...
use super::april_dawn_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("April Dawn.nksf")
}

#[test]
fn test_april_dawn_parse_success() {
...
#[test]
fn test_april_dawn_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
