...
use super::hyper_line_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Hyper Line.nksf")
}

#[test]
fn test_hyper_line_parse_success() {
...
#[test]
fn test_hyper_line_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
