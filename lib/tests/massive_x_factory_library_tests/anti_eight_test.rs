...
use super::anti_eight_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Anti Eight.nksf")
}

#[test]
fn test_anti_eight_parse_success() {
...
#[test]
fn test_anti_eight_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
