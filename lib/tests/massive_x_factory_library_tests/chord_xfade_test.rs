...
use super::chord_xfade_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Chord Xfade.nksf")
}

#[test]
fn test_chord_xfade_parse_success() {
...
#[test]
fn test_chord_xfade_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
