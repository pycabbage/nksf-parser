...
use super::altitude_crank_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Altitude Crank.nksf")
}

#[test]
fn test_altitude_crank_parse_success() {
...
#[test]
fn test_altitude_crank_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
