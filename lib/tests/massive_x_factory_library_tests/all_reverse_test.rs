...
use super::all_reverse_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("All Reverse.nksf")
}

#[test]
fn test_all_reverse_parse_success() {
...
#[test]
fn test_all_reverse_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
