...
use super::lovely_eighties_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Lovely Eighties.nksf")
}

#[test]
fn test_lovely_eighties_parse_success() {
...
#[test]
fn test_lovely_eighties_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
