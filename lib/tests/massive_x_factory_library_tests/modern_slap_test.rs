...
use super::modern_slap_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Modern Slap.nksf")
}

#[test]
fn test_modern_slap_parse_success() {
...
#[test]
fn test_modern_slap_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
