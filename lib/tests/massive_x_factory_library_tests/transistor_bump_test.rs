...
use super::transistor_bump_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Transistor Bump.nksf")
}

#[test]
fn test_transistor_bump_parse_success() {
...
#[test]
fn test_transistor_bump_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
