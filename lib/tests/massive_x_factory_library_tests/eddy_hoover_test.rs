...
use super::eddy_hoover_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Eddy Hoover.nksf")
}

#[test]
fn test_eddy_hoover_parse_success() {
...
#[test]
fn test_eddy_hoover_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
