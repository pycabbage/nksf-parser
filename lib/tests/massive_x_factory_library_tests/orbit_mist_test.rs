...
use super::orbit_mist_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Orbit Mist.nksf")
}

#[test]
fn test_orbit_mist_parse_success() {
...
#[test]
fn test_orbit_mist_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
