...
use super::bass_bandit_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Bass Bandit.nksf")
}

#[test]
fn test_bass_bandit_parse_success() {
...
#[test]
fn test_bass_bandit_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
