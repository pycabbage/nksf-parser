...
use super::wobbly_orbit_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Wobbly Orbit.nksf")
}

#[test]
fn test_wobbly_orbit_parse_success() {
...
#[test]
fn test_wobbly_orbit_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
