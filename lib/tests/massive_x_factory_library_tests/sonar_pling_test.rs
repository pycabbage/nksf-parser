...
use super::sonar_pling_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sonar Pling.nksf")
}

#[test]
fn test_sonar_pling_parse_success() {
...
#[test]
fn test_sonar_pling_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
