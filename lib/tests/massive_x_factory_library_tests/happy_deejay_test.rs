...
use super::happy_deejay_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Happy Deejay.nksf")
}

#[test]
fn test_happy_deejay_parse_success() {
...
#[test]
fn test_happy_deejay_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
