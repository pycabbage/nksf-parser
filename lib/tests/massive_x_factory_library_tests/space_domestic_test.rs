...
use super::space_domestic_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Space Domestic.nksf")
}

#[test]
fn test_space_domestic_parse_success() {
...
#[test]
fn test_space_domestic_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
