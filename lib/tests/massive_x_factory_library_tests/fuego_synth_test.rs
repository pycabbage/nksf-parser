...
use super::fuego_synth_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Fuego Synth.nksf")
}

#[test]
fn test_fuego_synth_parse_success() {
...
#[test]
fn test_fuego_synth_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
