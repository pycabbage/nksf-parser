...
use super::ghost_shadow_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ghost Shadow.nksf")
}

#[test]
fn test_ghost_shadow_parse_success() {
...
#[test]
fn test_ghost_shadow_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
