...
use super::digital_ghost_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Digital Ghost.nksf")
}

#[test]
fn test_digital_ghost_parse_success() {
...
#[test]
fn test_digital_ghost_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
