...
use super::peters_pan_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Peters Pan.nksf")
}

#[test]
fn test_peters_pan_parse_success() {
...
#[test]
fn test_peters_pan_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
