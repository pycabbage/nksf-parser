...
use super::retro_cruiser_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Retro Cruiser.nksf")
}

#[test]
fn test_retro_cruiser_parse_success() {
...
#[test]
fn test_retro_cruiser_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
