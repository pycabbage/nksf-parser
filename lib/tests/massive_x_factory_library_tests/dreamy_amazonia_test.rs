...
use super::dreamy_amazonia_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dreamy Amazonia.nksf")
}

#[test]
fn test_dreamy_amazonia_parse_success() {
...
#[test]
fn test_dreamy_amazonia_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
