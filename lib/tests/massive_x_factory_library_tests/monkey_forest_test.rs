...
use super::monkey_forest_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Monkey Forest.nksf")
}

#[test]
fn test_monkey_forest_parse_success() {
...
#[test]
fn test_monkey_forest_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
