...
use super::scrape_scape_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Scrape Scape.nksf")
}

#[test]
fn test_scrape_scape_parse_success() {
...
#[test]
fn test_scrape_scape_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
