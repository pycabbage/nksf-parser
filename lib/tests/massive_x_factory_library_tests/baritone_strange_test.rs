...
use super::baritone_strange_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Baritone Strange.nksf")
}

#[test]
fn test_baritone_strange_parse_success() {
...
#[test]
fn test_baritone_strange_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
