...
use super::hidden_monster_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Hidden Monster.nksf")
}

#[test]
fn test_hidden_monster_parse_success() {
...
#[test]
fn test_hidden_monster_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
