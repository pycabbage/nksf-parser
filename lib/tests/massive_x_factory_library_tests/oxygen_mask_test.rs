...
use super::oxygen_mask_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Oxygen Mask.nksf")
}

#[test]
fn test_oxygen_mask_parse_success() {
...
#[test]
fn test_oxygen_mask_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
