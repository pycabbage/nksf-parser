...
use super::hangry_gabo_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Hangry Gabo.nksf")
}

#[test]
fn test_hangry_gabo_parse_success() {
...
#[test]
fn test_hangry_gabo_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
