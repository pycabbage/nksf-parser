...
use super::happy_glide_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Happy Glide.nksf")
}

#[test]
fn test_happy_glide_parse_success() {
...
#[test]
fn test_happy_glide_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
