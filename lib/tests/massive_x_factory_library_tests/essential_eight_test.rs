...
use super::essential_eight_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Essential Eight.nksf")
}

#[test]
fn test_essential_eight_parse_success() {
...
#[test]
fn test_essential_eight_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
