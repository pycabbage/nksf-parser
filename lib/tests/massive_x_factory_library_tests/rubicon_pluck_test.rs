...
use super::rubicon_pluck_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Rubicon Pluck.nksf")
}

#[test]
fn test_rubicon_pluck_parse_success() {
...
#[test]
fn test_rubicon_pluck_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
