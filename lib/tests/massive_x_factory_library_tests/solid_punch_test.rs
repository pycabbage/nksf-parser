...
use super::solid_punch_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Solid Punch.nksf")
}

#[test]
fn test_solid_punch_parse_success() {
...
#[test]
fn test_solid_punch_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
