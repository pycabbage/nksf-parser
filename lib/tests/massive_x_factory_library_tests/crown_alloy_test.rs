...
use super::crown_alloy_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Crown Alloy.nksf")
}

#[test]
fn test_crown_alloy_parse_success() {
...
#[test]
fn test_crown_alloy_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
