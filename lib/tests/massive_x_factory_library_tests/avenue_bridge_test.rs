...
use super::avenue_bridge_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Avenue Bridge.nksf")
}

#[test]
fn test_avenue_bridge_parse_success() {
...
#[test]
fn test_avenue_bridge_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
