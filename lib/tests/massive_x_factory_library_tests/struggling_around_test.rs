...
use super::struggling_around_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Struggling Around.nksf")
}

#[test]
fn test_struggling_around_parse_success() {
...
#[test]
fn test_struggling_around_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_struggling_around_nisi() {
    let path = get_fixture_path();
