...
use super::multi_phasor_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Multi Phasor.nksf")
}

#[test]
fn test_multi_phasor_parse_success() {
...
#[test]
fn test_multi_phasor_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
