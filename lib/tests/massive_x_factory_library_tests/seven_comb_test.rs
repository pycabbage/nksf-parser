...
use super::seven_comb_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Seven Comb.nksf")
}

#[test]
fn test_seven_comb_parse_success() {
...
#[test]
fn test_seven_comb_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
