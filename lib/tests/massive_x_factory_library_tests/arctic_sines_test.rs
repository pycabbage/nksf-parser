...
use super::arctic_sines_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Arctic Sines.nksf")
}

#[test]
fn test_arctic_sines_parse_success() {
...
#[test]
fn test_arctic_sines_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
