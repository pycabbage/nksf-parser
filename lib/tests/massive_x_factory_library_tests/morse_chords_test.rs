...
use super::morse_chords_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Morse Chords.nksf")
}

#[test]
fn test_morse_chords_parse_success() {
...
#[test]
fn test_morse_chords_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
