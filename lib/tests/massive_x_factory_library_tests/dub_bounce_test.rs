...
use super::dub_bounce_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Dub Bounce.nksf")
}

#[test]
fn test_dub_bounce_parse_success() {
...
#[test]
fn test_dub_bounce_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
