...
use super::melodic_ground_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Melodic Ground.nksf")
}

#[test]
fn test_melodic_ground_parse_success() {
...
#[test]
fn test_melodic_ground_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
