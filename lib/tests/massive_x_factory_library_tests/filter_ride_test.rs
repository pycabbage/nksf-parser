...
use super::filter_ride_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Filter Ride.nksf")
}

#[test]
fn test_filter_ride_parse_success() {
...
#[test]
fn test_filter_ride_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
