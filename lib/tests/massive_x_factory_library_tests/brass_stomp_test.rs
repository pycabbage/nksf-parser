...
use super::brass_stomp_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Brass Stomp.nksf")
}

#[test]
fn test_brass_stomp_parse_success() {
...
#[test]
fn test_brass_stomp_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
