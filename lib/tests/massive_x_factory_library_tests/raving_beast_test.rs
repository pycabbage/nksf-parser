...
use super::raving_beast_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Raving Beast.nksf")
}

#[test]
fn test_raving_beast_parse_success() {
...
#[test]
fn test_raving_beast_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
