...
use super::smash_piano_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Smash Piano.nksf")
}

#[test]
fn test_smash_piano_parse_success() {
...
#[test]
fn test_smash_piano_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
