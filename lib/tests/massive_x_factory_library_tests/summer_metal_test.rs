...
use super::summer_metal_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Summer Metal.nksf")
}

#[test]
fn test_summer_metal_parse_success() {
...
#[test]
fn test_summer_metal_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
