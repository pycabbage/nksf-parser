...
use super::melancholic_rims_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Melancholic Rims.nksf")
}

#[test]
fn test_melancholic_rims_parse_success() {
...
#[test]
fn test_melancholic_rims_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
