...
use super::midtown_swirl_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Midtown Swirl.nksf")
}

#[test]
fn test_midtown_swirl_parse_success() {
...
#[test]
fn test_midtown_swirl_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
