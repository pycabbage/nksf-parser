...
use super::data_transmissions_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Data Transmissions.nksf")
}

#[test]
fn test_data_transmissions_parse_success() {
...
#[test]
fn test_data_transmissions_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_data_transmissions_nisi() {
    let path = get_fixture_path();
