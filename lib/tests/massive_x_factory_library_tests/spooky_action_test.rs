...
use super::spooky_action_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Spooky Action.nksf")
}

#[test]
fn test_spooky_action_parse_success() {
...
#[test]
fn test_spooky_action_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
