...
use super::plucked_up_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Plucked Up.nksf")
}

#[test]
fn test_plucked_up_parse_success() {
...
#[test]
fn test_plucked_up_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
