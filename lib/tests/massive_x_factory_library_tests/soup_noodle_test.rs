...
use super::soup_noodle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Soup Noodle.nksf")
}

#[test]
fn test_soup_noodle_parse_success() {
...
#[test]
fn test_soup_noodle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
