...
use super::steamy_strike_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Steamy Strike.nksf")
}

#[test]
fn test_steamy_strike_parse_success() {
...
#[test]
fn test_steamy_strike_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
