...
use super::barely_dead_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Barely Dead.nksf")
}

#[test]
fn test_barely_dead_parse_success() {
...
#[test]
fn test_barely_dead_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
