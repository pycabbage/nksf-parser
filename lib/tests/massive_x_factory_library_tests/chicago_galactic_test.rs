...
use super::chicago_galactic_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Chicago Galactic.nksf")
}

#[test]
fn test_chicago_galactic_parse_success() {
...
#[test]
fn test_chicago_galactic_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
