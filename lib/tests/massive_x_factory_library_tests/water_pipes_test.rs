...
use super::water_pipes_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Water Pipes.nksf")
}

#[test]
fn test_water_pipes_parse_success() {
...
#[test]
fn test_water_pipes_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
