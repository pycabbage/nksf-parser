...
use super::belly_stage_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Belly Stage.nksf")
}

#[test]
fn test_belly_stage_parse_success() {
...
#[test]
fn test_belly_stage_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
