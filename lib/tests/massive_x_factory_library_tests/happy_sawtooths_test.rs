...
use super::happy_sawtooths_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Happy Sawtooths.nksf")
}

#[test]
fn test_happy_sawtooths_parse_success() {
...
#[test]
fn test_happy_sawtooths_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
