...
use super::the_arpology_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("The Arpology.nksf")
}

#[test]
fn test_the_arpology_parse_success() {
...
#[test]
fn test_the_arpology_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
