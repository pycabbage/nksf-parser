...
use super::birdy_hats_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Birdy Hats.nksf")
}

#[test]
fn test_birdy_hats_parse_success() {
...
#[test]
fn test_birdy_hats_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
