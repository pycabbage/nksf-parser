...
use super::tight_combo_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Tight Combo.nksf")
}

#[test]
fn test_tight_combo_parse_success() {
...
#[test]
fn test_tight_combo_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
