...
use super::auto_chord_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Auto Chord.nksf")
}

#[test]
fn test_auto_chord_parse_success() {
...
#[test]
fn test_auto_chord_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
