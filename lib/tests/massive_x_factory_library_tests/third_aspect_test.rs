...
use super::third_aspect_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Third Aspect.nksf")
}

#[test]
fn test_third_aspect_parse_success() {
...
#[test]
fn test_third_aspect_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
