...
use super::sine_cloud_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Sine Cloud.nksf")
}

#[test]
fn test_sine_cloud_parse_success() {
...
#[test]
fn test_sine_cloud_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
