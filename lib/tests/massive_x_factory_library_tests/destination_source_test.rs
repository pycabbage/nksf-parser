...
use super::destination_source_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Destination Source.nksf")
}

#[test]
fn test_destination_source_parse_success() {
...
#[test]
fn test_destination_source_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_destination_source_nisi() {
    let path = get_fixture_path();
