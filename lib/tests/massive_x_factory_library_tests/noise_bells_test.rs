...
use super::noise_bells_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Noise Bells.nksf")
}

#[test]
fn test_noise_bells_parse_success() {
...
#[test]
fn test_noise_bells_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
