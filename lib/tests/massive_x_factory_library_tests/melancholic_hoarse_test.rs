...
use super::melancholic_hoarse_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Melancholic Hoarse.nksf")
}

#[test]
fn test_melancholic_hoarse_parse_success() {
...
#[test]
fn test_melancholic_hoarse_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_melancholic_hoarse_nisi() {
    let path = get_fixture_path();
