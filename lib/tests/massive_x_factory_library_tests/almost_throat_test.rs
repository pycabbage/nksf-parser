...
use super::almost_throat_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Almost Throat.nksf")
}

#[test]
fn test_almost_throat_parse_success() {
...
#[test]
fn test_almost_throat_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
