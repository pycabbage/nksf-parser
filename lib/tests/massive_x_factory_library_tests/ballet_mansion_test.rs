...
use super::ballet_mansion_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ballet Mansion.nksf")
}

#[test]
fn test_ballet_mansion_parse_success() {
...
#[test]
fn test_ballet_mansion_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
