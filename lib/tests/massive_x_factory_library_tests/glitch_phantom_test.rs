...
use super::glitch_phantom_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Glitch Phantom.nksf")
}

#[test]
fn test_glitch_phantom_parse_success() {
...
#[test]
fn test_glitch_phantom_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
