...
use super::pulse_rider_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Pulse Rider.nksf")
}

#[test]
fn test_pulse_rider_parse_success() {
...
#[test]
fn test_pulse_rider_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
