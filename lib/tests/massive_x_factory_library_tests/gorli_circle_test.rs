...
use super::gorli_circle_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Gorli Circle.nksf")
}

#[test]
fn test_gorli_circle_parse_success() {
...
#[test]
fn test_gorli_circle_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
