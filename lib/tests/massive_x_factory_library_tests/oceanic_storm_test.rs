...
use super::oceanic_storm_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Oceanic Storm.nksf")
}

#[test]
fn test_oceanic_storm_parse_success() {
...
#[test]
fn test_oceanic_storm_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
