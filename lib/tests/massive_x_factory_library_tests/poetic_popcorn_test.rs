...
use super::poetic_popcorn_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Poetic Popcorn.nksf")
}

#[test]
fn test_poetic_popcorn_parse_success() {
...
#[test]
fn test_poetic_popcorn_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
