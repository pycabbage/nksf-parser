...
use super::expensive_tastes_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Expensive Tastes.nksf")
}

#[test]
fn test_expensive_tastes_parse_success() {
...
#[test]
fn test_expensive_tastes_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
