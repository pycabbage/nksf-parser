...
use super::ensemble_choir_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Ensemble Choir.nksf")
}

#[test]
fn test_ensemble_choir_parse_success() {
...
#[test]
fn test_ensemble_choir_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
