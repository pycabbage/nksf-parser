...
use super::orchestra_bubbles_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Orchestra Bubbles.nksf")
}

#[test]
fn test_orchestra_bubbles_parse_success() {
...
#[test]
fn test_orchestra_bubbles_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
...
#[test]
fn test_orchestra_bubbles_nisi() {
    let path = get_fixture_path();
