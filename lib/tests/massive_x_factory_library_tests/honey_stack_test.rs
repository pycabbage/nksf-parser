...
use super::honey_stack_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Honey Stack.nksf")
}

#[test]
fn test_honey_stack_parse_success() {
...
#[test]
fn test_honey_stack_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
