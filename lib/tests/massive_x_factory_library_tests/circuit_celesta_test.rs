...
use super::circuit_celesta_expected_data as expected;
...
fn get_fixture_path() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("fixture")
        .join("Circuit Celesta.nksf")
}

#[test]
fn test_circuit_celesta_parse_success() {
...
#[test]
fn test_circuit_celesta_complete_parse() {
    let path = get_fixture_path();
    let result = parse_nksf(&path);
    match result {
        Ok(_) => { /* OK */ }
