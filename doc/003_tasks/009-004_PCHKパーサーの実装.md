...
- [x] flate2クレートを本番依存関係として追加
- [x] `lib/src/types.rs` に `PchkData` 構造体を追加
- [x] `lib/src/types.rs` に `PchkHeader` 構造体を追加
- [x] `lib/src/pchk_parser.rs` を作成
- [x] PCHKパーサーをメインパーサー（parser.rs）に統合
- [x] `NksfFile` 構造体に `plugin_chunk: PchkData` フィールドを追加
- [x] `unknown_chunks` から PCHK を削除するロジックを実装
- [x] ユニットテストを `pchk_parser.rs` に追加
- [x] 統合テストを `abandoned_test.rs` に追加（test_abandoned_pchk）
- [x] 既存テスト（test_abandoned_unknown_chunks）を修正
- [x] lib.rsにモジュール宣言とエクスポート追加
