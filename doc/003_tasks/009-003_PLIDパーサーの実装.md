...
- [x] `lib/src/types.rs` に `PlidData` 構造体を追加
- [x] `lib/src/plid_parser.rs` を作成
- [x] PLIDパーサーをメインパーサー（parser.rs）に統合
- [x] `NksfFile` 構造体に `plugin_id: PlidData` フィールドを追加
- [x] `unknown_chunks` から PLID を削除するロジックを実装
- [x] ユニットテストを `plid_parser.rs` に追加
- [x] 統合テストを `abandoned_test.rs` に追加（test_abandoned_plid）
